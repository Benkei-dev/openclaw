...
- [~CC-HAIKU] TASK-20 🟢 `CC-HAIKU`: **WF4 Auth-Fix + Telegram-Spam stoppen**. DRINGEND! WF4 Bridge Health Node sendet GET ohne Auth-Header → 401 → "Bridge offline" Telegram-Spam alle 5min. Fix: Auth-Header `Authorization: Bearer 77cc86ea...28cb` zum HTTP-Node `Bridge Health` hinzufügen. Danach IF-Node prüft `status == ok` korrekt.
